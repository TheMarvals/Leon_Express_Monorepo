...
echo "🛠️  Re-construyendo backend..."
...
scp -i ssh_key/id_rsa.priv -o StrictHostKeyChecking=no leon_express-backend.tar.gz root@65.75.201.175:/opt/leon_express/

echo "🔄 Reiniciando contenedor en servidor..."
ssh -i ssh_key/id_rsa.priv -o StrictHostKeyChecking=no root@65.75.201.175 "
  cd /opt/leon_express
  docker load -i leon_express-backend.tar.gz
