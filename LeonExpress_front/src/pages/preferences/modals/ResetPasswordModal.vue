    ...
    <VaForm ref="form" class="space-y-6" @submit.prevent="submit">
      ...
      <div class="flex flex-col space-y-2">
        <div class="flex space-x-2 items-center">
          <div>
            <VaIcon
              :name="newPassword?.length! >= 8 ? 'check_circle' : 'cancel'"
              :color="newPassword?.length! >= 8 ? 'success' : 'danger'"
              size="20px"
            />
          </div>
        ...
        <div class="flex space-x-2 items-center">
          <div>
            <VaIcon
              :name="new Set(newPassword).size >= 6 ? 'check_circle' : 'cancel'"
              :color="new Set(newPassword).size >= 6 ? 'success' : 'danger'"
              size="20px"
            />
          </div>
...
const submit = async () => {
  ...
  try {
    await api.changePassword({
      currentPassword: oldPassowrd.value!,
      newPassword: newPassword.value!,
      confirmNewPassword: repeatNewPassword.value!,
    })

    init({ message: 'Has cambiado tu contraseña exitosamente', color: 'success' })
    emits('cancel')
  } catch (error: any) {
