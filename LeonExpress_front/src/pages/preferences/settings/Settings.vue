  ...
  <div class="flex flex-col md:flex-row md:items-center space-y-2 md:space-y-0 md:space-x-6 min-h-[36px] leading-5">
    ...
    <div class="flex-1">
      <div class="max-w-[748px]">
        {{ store.user?.full_name || store.user?.fullname || 'No disponible' }}
      </div>
    </div>
...
// import { useToast } from 'vuestic-ui' // Comentado hasta implementar 2FA
...
// const { init } = useToast() // Comentado hasta implementar 2FA

// Lógica del 2FA comentada hasta su implementación completa
/*
// Simulamos el estado del 2FA (no implementado aún)
const is2FAEnabled = ref(false)

const toastMessage = computed(() => (is2FAEnabled.value ? '2FA successfully enabled' : '2FA successfully disabled'))
